...
FROM thegrint_grint.grint_user
	...
	LEFT JOIN (
		SELECT
			...
			1 AS whs_user,
			CASE
				WHEN DATEDIFF(MIN(p.purchase_date), user.doj) <= 60 THEN 1
				ELSE 0
			END AS whs_user_60_days
		FROM thegrint_grint.whs_transition_user u
			LEFT JOIN thegrint_grint.grint_user user ON u.user_id = user.user_id
			LEFT JOIN thegrint_grint.whs_membership_payment p on u.ghin = p.ghin
		GROUP BY u.user_id
	...
	LEFT JOIN (
		SELECT
			...
			COUNT(*) AS friends,
			COUNT(DISTINCT CASE
						   WHEN DATEDIFF(f.created_at, g.doj) <= 60 THEN f.user_id
			END) AS friends_60_days
