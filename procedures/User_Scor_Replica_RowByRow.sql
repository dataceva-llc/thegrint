CREATE DEFINER=`1bT12@uylp12`@`%` PROCEDURE `User_Scor_Replica_RowByRow`()
BEGIN
...
DECLARE done INT DEFAULT FALSE;
   ...
   -- get max scoreid in the replica table
    SELECT COALESCE(MAX(scoreid), 0) INTO max_scoreid FROM thegrint_analytics.user_score_replica;
    OPEN score_cursor;
    ...
    read_loop: LOOP
        ...
        END IF;
         ...
         INSERT INTO thegrint_analytics.user_score_replica (
            scoreid,
			userid,
			creater,
			creater_status,
          ...
          END LOOP;
    CLOSE score_cursor;
    	-- to delete score_id from "thegrint_analytics.user_score_replica" which are not present in "thegrint_grint.user_score"
DELETE FROM thegrint_analytics.user_score_replica
WHERE NOT EXISTS (
  SELECT 1 FROM thegrint_grint.user_score
  WHERE user_score.scoreId = user_score_replica.scoreId
);
