...
CREATE DEFINER = `1bT12@uylp12`@`%` PROCEDURE thegrint_analytics.update_user_friends()
BEGIN
/**
* Update or create the user_all_scores table with the latest score data for both 18 and 9 holes.
* RUN TIME: ~1200 seconds (20 minutes)
*
...
*
* On first create, run the following query to index the user_all_scores table:
*   CREATE INDEX user_all_scores_score_id ON thegrint_analytics.user_all_scores (score_id);
*   CREATE INDEX user_all_scores_user_id ON thegrint_analytics.user_all_scores (user_id);
*/

    -- Drop the user_membership table if it exist
    DROP TABLE IF EXISTS thegrint_analytics.user_friends;

    -- Create the table and insert the records
    CREATE TABLE thegrint_analytics.user_friends AS
    SELECT
